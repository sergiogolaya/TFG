% Compute representative EMG curves using DTW-aligned combined data
...
save_dir = 'C:\Users\sergi\Documents\CEU\TFG\results\dtw_mean';
...
for i = 1:length(file_list)
    ...
    if ~isfield(loaded_data, 'combined_emg_struct')
        warning('combined_emg_struct not found in file: %s', file_list(i).name);
        continue;
    end

    % Access the combined structure
    ...
    if i == 1
        ...
        num_samples = size(emg_struct.(muscle_fields{1}).envelope, 1); % Assuming same length for all muscles
...
for m = 1:num_muscles
    ...
    muscle_mean = mean(subject_data(:, :, m), 2, 'omitnan');
    muscle_std = std(subject_data(:, :, m), 0, 2, 'omitnan');
    ...
    saveas(gcf, fullfile(save_dir, [muscle_fields{m}, '_dtw_emg_curve.png']));
    print(gcf, fullfile(save_dir, [muscle_fields{m}, '_dtw_emg_curve.png']), '-dpng', '-r300'); % Save with 300 DPI
    close(gcf);
end

% Save the structured EMG results in a .mat file
save(fullfile(save_dir, 'representative_dtw_emg_curve.mat'), 'emg_results');

disp('Representative DTW EMG curves computed and saved with high resolution.');
