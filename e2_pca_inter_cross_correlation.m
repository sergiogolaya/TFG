...
for m = 1:length(muscles)
    ...
    if show_plots
        ...
        colormap jet;
        colorbar;
        title(['Cross-Correlation Between Subjects (PCA) - ' muscle_name]);
        xlabel('Subject');
        ylabel('Subject');
...
% Save results
save_filename = fullfile(save_dir, 'pca_crossCorrResults.mat');
save(save_filename, 'crossCorrResults');

fprintf('PCA cross-correlation results saved to: %s\n', save_filename);